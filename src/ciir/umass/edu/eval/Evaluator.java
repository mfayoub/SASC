...
import ciir.umass.edu.metric.CSDCGScorer;
import ciir.umass.edu.metric.ERRScorer;
import ciir.umass.edu.metric.METRIC;
import ciir.umass.edu.metric.MetricScorer;
import ciir.umass.edu.metric.MetricScorerFactory;
import ciir.umass.edu.metric.PCSDCGScorer;
 ...
 *         data, etc.) as well as algorithm-specific parameters. Type "java -jar
 *         bin/RankLib.jar" at the command-line to see all the options.
 */
public class Evaluator {

	static boolean applySensitivityPredictions = false;
	...
	public static void main(String[] args) {
		...
		String[] rType = new String[] { "MART", "RankNet", "RankBoost", "AdaRank", "Coordinate Ascent", "LambdaRank",
				"LambdaMART", "ListNet", "Random Forests", "Linear Regression", "BM25", "Filtered MART",
				"Filtered RankNet", "Filtered RankBoost", "Filtered AdaRank", "Filtered Coordinate Ascent",
				"Filtered LambdaRank", "Filtered LambdaMART", "Filtered ListNet", "Filtered Random Forests",
				"Filtered Linear Regression", "Filtered BM25" };
		RANKER_TYPE[] rType2 = new RANKER_TYPE[] { RANKER_TYPE.MART, RANKER_TYPE.RANKNET, RANKER_TYPE.RANKBOOST,
				RANKER_TYPE.ADARANK, RANKER_TYPE.COOR_ASCENT, RANKER_TYPE.LAMBDARANK, RANKER_TYPE.LAMBDAMART,
				RANKER_TYPE.LISTNET, RANKER_TYPE.RANDOM_FOREST, RANKER_TYPE.LINEAR_REGRESSION, RANKER_TYPE.BM25 };
		...
		if (args.length < 2) {
			...
			// System.out.println("\t[ -metric2t <metric> ]\tMetric to optimize on the
			// training data. Supported: MAP, NDCG@k, DCG@k, P@k, RR@k, BEST@k, ERR@k
			// (default=" + trainMetric + ")");
			System.out.println(
			...
			// System.out.println("");
			// System.out.println("\t[ -sparse ]\t\tUse sparse representation for all
			// feature vectors (default=dense)");
			...
			// System.out.println("\t\t\t\tData for each fold is created from sequential
			// partitions of the training data.");
			// System.out.println("\t\t\t\tRandomized partitioning can be done by shuffling
			// the training data in advance.");
			// System.out.println("\t\t\t\tType \"java -cp bin/RankLib.jar
			// ciir.umass.edu.feature.FeatureManager\" for help with shuffling.");
			...
			System.out
					.println("\t[ -epoch <T> ]\t\tThe number of epochs to train (default=" + RankNet.nIteration + ")");
			System.out.println(
					"\t[ -layer <layer> ]\tThe number of hidden layers (default=" + RankNet.nHiddenLayer + ")");
			System.out.println("\t[ -node <node> ]\tThe number of hidden nodes per layer (default="
			...
			System.out
					.println("\t[ -round <T> ]\t\tThe number of rounds to train (default=" + AdaRank.nIteration + ")");
			System.out.println("\t[ -noeq ]\t\tTrain without enqueuing too-strong features (default=unspecified)");
			System.out.println("\t[ -tolerance <t> ]\tTolerance between two consecutive rounds of learning (default="
					+ AdaRank.tolerance + ")");
			System.out.println(
			...
			System.out.println("\t[ -i <iteration> ]\tThe number of iterations to search in each dimension (default="
					+ CoorAscent.nMaxIteration + ")");
			System.out.println("\t[ -tolerance <t> ]\tPerformance tolerance between two solutions (default="
			...
			System.out.println(
					"\t[ -mls <n> ]\t\tMin leaf support -- minimum % of docs each leaf has to contain (default="
			...
			System.out
					.println("\t[ -epoch <T> ]\t\tThe number of epochs to train (default=" + ListNet.nIteration + ")");
			System.out.println("\t[ -lr <rate> ]\t\tLearning rate (default="
		...
		for (int i = 0; i < args.length; i++) {
			...
			/////////////////////////////////////////////////////
				...
				mustHaveRelDoc = true;
			else if (args[i].compareTo("-sensek") == 0) {
				CSDCGScorer.SENSITIVITY_CONSTANT = Integer.parseInt(args[i + 1]);
				PCSDCGScorer.SENSITIVITY_CONSTANT = Integer.parseInt(args[++i]);
			} else if (args[i].compareTo("-applysensepred") == 0) {
				applySensitivityPredictions = true;
			} else {
		...
		System.out.println("");
		// System.out.println((keepOrigFeatures)?"Keep orig. features":"Discard orig.
		// features");
		...
		if (trainFile.compareTo("") != 0) {
			...
			if (foldCV != -1) {
				...
				// - Models saved if EITHER kcvmd OR kcvmn defined. Use default names for
				// missing values.
				...
				if (ttSplit > 0.0)// we should use a held-out portion of the training data for testing?
					e.evaluate(trainFile, validationFile, featureDescriptionFile, ttSplit);// no validation will be done
																							// if validationFile=""
					...
					e.evaluate(trainFile, validationFile, testFile, featureDescriptionFile);// All files except for
		...
		} else // scenario: test a saved model
		{
			...
			if (rankFile.compareTo("") != 0) {
				if (scoreFile.compareTo("") != 0) {
					...
					// It will evaluate the input ranking (without being re-ranked by any model)
					// using any measure specified via metric2T
					else
	...
	// The relevance labels attached to our samples might be only a subset of the
	// entire relevance judgment set.
	// If we're working on datasets like Letor/Web10K or Yahoo! LTR, we can totally
	...
	// there's a high chance some relevant document (the in qrel file TREC provides)
	// does not appear in our top-K list -- thus the calculation of
	// MAP and NDCG is no longer precise. If so, specify that "external" relevance
	// judgment here (via the -qrel cmd parameter)
	...
	public double evaluate(Ranker ranker, List<RankList> rl) {
		List<RankList> l = rl;
		if (ranker != null)
			l = ranker.rank(rl, applySensitivityPredictions);
	...
	/**
	 * Evaluate the currently selected ranking algorithm using percenTrain% of the
	 * samples for training the rest for testing.
	 * 
	 * @param sampleFile
	...
	/**
	 * Evaluate the currently selected ranking algorithm using percenTrain% of the
	 * training samples for training the rest as validation data. Test data is
	 ...
	 * @param modelFile
	 */
	public void evaluate(String sampleFile, String featureDefFile, int nFold, float tvs, String modelDir,
			String modelFile) {
		...
		System.out.println("----------------------------------");
		for (int i = 0; i < nFold; i++)
			System.out.println("Fold " + (i + 1) + "\t|   " + SimpleMath.round(scores[i][0], 4) + "\t|  "
					+ SimpleMath.round(scores[i][1], 4) + "\t");
		System.out.println("----------------------------------");
		System.out.println("Avg.\t|   " + SimpleMath.round(scoreOnTrain / nFold, 4) + "\t|  "
				+ SimpleMath.round(scoreOnTest / nFold, 4) + "\t");
		System.out.println("----------------------------------");
		System.out.println(
				"Total\t|   " + "\t" + "\t|  " + SimpleMath.round(totalScoreOnTest / totalTestSampleSize, 4) + "\t");
	}
	...
	/**
	 * Evaluate the performance (in -metric2T) of a pre-trained model. Save its
	 * performance on each of the ranked list if this is specified.
	 * 
	 * @param modelFile Pre-trained model
	 * @param testFile  Test data
	 * @param prpFile   Per-ranked list performance file: Model's performance on
	 *                  each of the ranked list. These won't be saved if prpFile="".
	 */
	public void test(String modelFile, String testFile, String prpFile) {
		...
		for (RankList aTest : test) {
			RankList l = ranker.rank(aTest, applySensitivityPredictions);
			double score = testScorer.score(l);
			ids.add(l.getID());
			scores.add(score);
			rankScore += score;
	...
	/**
	 * Evaluate the performance (in -metric2T) of k pre-trained models. Data in the
	 * test file will be splitted into k fold, where k=|models|. Each model will be
	 * evaluated on the data from the corresponding fold.
	 * 
	 * @param modelFiles Pre-trained models
	 * @param testFile   Test data
	 * @param prpFile    Per-ranked list performance file: Model's performance on
	 *                   each of the ranked list. These won't be saved if
	 *                   prpFile="".
	 */
	public void test(List<String> modelFiles, String testFile, String prpFile) {
		...
		for (int f = 0; f < nFold; f++) {
			...
			for (int i = 0; i < test.size(); i++) {
				RankList l = ranker.rank(test.get(i), applySensitivityPredictions);
				double score = testScorer.score(l);
				ids.add(l.getID());
				scores.add(score);
				rankScore += score;
	...
	/**
	 * Similar to the above, except data has already been splitted. The k-th model
	 * will be applied on the k-th test file.
	 * 
	 * @param modelFiles
	 * @param testFiles
	 * @param prpFile
	...
	/**
	 * Write the models' score for each of the documents in a test rankings. These
	 * test rankings are splitted into k chunks where k=|models|. Each model is
	 * applied on the data from the corresponding fold.
	 * 
	 * @param modelFiles
	 * @param testFile
	 * @param outputFile
	...
	/**
	 * Use a pre-trained model to re-rank the test rankings. Save the output ranking
	 * in indri's run format
	 * 
	 * @param modelFile
	 * @param testFile
	 * @param indriRanking
	...
	public void rank(String modelFile, String testFile, String indriRanking) {
		...
		try {
			BufferedWriter out = new BufferedWriter(
					new OutputStreamWriter(new FileOutputStream(indriRanking), "UTF-8"));
			for (RankList l : test) {
	...
	public void rank(String testFile, String indriRanking) {
		...
		try {
			...
			for (RankList l : test) {
				for (int j = 0; j < l.size(); j++) {
					String str = l.getID() + " Q0 " + l.get(j).getDescription().replace("#", "").trim() + " " + (j + 1)
	...
	/**
	 * Use k pre-trained models to re-rank the test rankings. Test rankings will be
	 * splitted into k fold, where k=|models|. Each model will be used to rank the
	 * data from the corresponding fold. Save the output ranking in indri's run
	 * format.
	 * 
	 * @param modelFiles
	 * @param testFile
	 ...
	 * @param featureDefFile Feature definition file (if it's an empty string, all
	 *                       features in the input file will be used)
	 * @param percentTrain   How much of the input data will be used for training?
	 *                       (the remaining will be reserved for test/validation)
	 ...
	 * @param scores  Evaluation score (in whatever measure specified/calculated
