...
public class AdaRank extends Ranker {
	...
	public static int maxSelCount = 5;// the max. number of times a feature can be selected consecutively before being
										// removed
		...
		for (; t <= nIteration; t++) {
			...
			if (withEnqueue) {
				if (bestWR.getFID() == lastFeature)// this feature is selected twice in a row
				{
					...
					// roll back the previous weak ranker since it is based on this "too strong"
					// feature
					...
					PRINTLN(new int[] { 8, 12, 12, 9 }, new String[] { bestWR.getFID() + "", "", "", "ROLLBACK" });
					continue;
				} else {
			...
			for (RankList sample : samples) {
				double tmp = scorer.score(rank(sample, false));
			...
			PRINT(new int[] { 8, 12, }, new String[] { bestWR.getFID() + "", SimpleMath.round(trainedScore, 4) + "" });
			if (t % 1 == 0 && validationSamples != null) {
				double scoreOnValidation = scorer.score(rank(validationSamples, false));
				if (scoreOnValidation > bestScoreOnValidationData) {
					bestScoreOnValidationData = scoreOnValidation;
					updateBestModelOnValidation();
				}
				PRINT(new int[] { 12, 9 }, new String[] { SimpleMath.round(scoreOnValidation, 4) + "", status });
			} else
				PRINT(new int[] { 12, 9 }, new String[] { "", status });
			PRINTLN("");

			//System.out.println("mfayoub delta " + delta);
			...
			for (int i = 0; i < sweight.length; i++)
				sweight[i] *= Math.exp(-alpha_t * scorer.score(rank(samples.get(i), false))) / total;
		}
		return t;
	}
	...
	public void learn() {
		...
		PRINTLN("--------------------------------------------------------");
		PRINTLN(new int[] { 7, 8, 12, 12, 9 },
				new String[] { "#iter", "Sel. F.", scorer.name() + "-T", scorer.name() + "-V", "Status" });
		PRINTLN("--------------------------------------------------------");

		System.out.println("Training with enqueue " + trainWithEnqueue);
		...
		scoreOnTrainingData = SimpleMath.round(scorer.score(rank(samples, false)), 4);
		PRINTLN("--------------------------------------------------------");
		PRINTLN("Finished sucessfully.");
		PRINTLN(scorer.name() + " on training data: " + scoreOnTrainingData);
		if (validationSamples != null) {
			bestScoreOnValidationData = scorer.score(rank(validationSamples, false));
