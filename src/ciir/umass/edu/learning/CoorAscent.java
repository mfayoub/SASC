...
public class CoorAscent extends Ranker {
	...
	public void learn()
	{
		...
		for(int r=0;r<nRestart;r++)
		{
			...
			current_feature = -1;
			double startScore = scorer.score(rank(samples, false));//compute all the scores (in whatever metric specified) and store them as cache
				...
				for(int i=0;i<fids.length;i++)
				{
					...
					for(int s=0;s<sign.length;s++)//search by both increasing and decreasing
					{
						...
						for(int j=0;j<nMaxIteration;j++)
						{
							...
							weight[fids[i]] = w;
							double score = scorer.score(rank(samples, false));
							if(regularized)
							{
			...
			{
				current_feature = -1;
				bestScore = scorer.score(rank(validationSamples, false));
			}			
