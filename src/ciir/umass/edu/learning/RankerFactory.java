...
public class RankerFactory {
	...
	protected Ranker[] rFactory = new Ranker[] { new MART(), new RankBoost(), new RankNet(), new AdaRank(),
			new CoorAscent(), new LambdaRank(), new LambdaMART(), new ListNet(), new RFRanker(), new LinearRegRank(),
			new BM25Rank() };
	...
	public RankerFactory() {
		...
		map.put(createRanker(RANKER_TYPE.BM25).name().toUpperCase(), RANKER_TYPE.BM25);
	}
	...
	public Ranker createRanker(String className) {
		...
		try {
			...
			System.out.println("Could find the class \"" + className
					+ "\" you specified. Make sure the jar library is in your classpath.");
			e.printStackTrace();
			System.exit(1);
