...
public class RankNet extends Ranker {
	...
	// mfayoub: It substitutes all documents features and constructs all possible pairs
	// pair[doc1][some count] = doc2 --> means doc1 is before (or has higher rank than) doc2
	...
	{
		for(int k=0;k<layers.size();k++)//skip the input layer ... mfayoub: is the comment wrong ? Cuz k starts from 0 which is the input layer
	...
	public void learn()
	{
		...
		for(int i=1;i<=nIteration;i++)
		{
			...
			//printWeightVector();
			scoreOnTrainingData = scorer.score(rank(samples, false));
			estimateLoss();
			...
			if(i % 1 == 0)
			{
				...
				if(validationSamples != null)
				{
					double score = scorer.score(rank(validationSamples, false));
